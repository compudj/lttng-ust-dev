			   ...
			   struct shm_object *shmobj, bool preallocate_backing)
{
	...
	ret = lib_ring_buffer_backend_create(&buf->backend, &chan->backend,
			cpu, handle, shmobj, preallocate_backing);
	if (ret) {
				    ...
				    struct lttng_ust_shm_handle *handle)
{
	 ...
	 * this sub-buffer's timestamp_end. There are also no concurrent
	 * readers of that timestamp_end because delivery of that
	...
	/*
	 * Order all writes to buffer and store to timestamp_end before
	 * the commit count update that will determine that the
	 * subbuffer is full.
	 */
	cmm_smp_wmb();
	cc_hot = shmp_index(handle, buf->commit_hot, oldidx);
				   ...
				   const struct lttng_ust_ring_buffer_ctx *ctx)
{
	...
	if (caa_likely(v_cmpxchg(config, &cc_cold->cc_sb,
		 ...
		 *
		 * We can read the timestamp_end for the current sub-buffer
		 * which has been saved by the very last space
		 * reservation for the current sub-buffer.
		 *
		 * Order increment of commit counter before reading
		 * timestamp_end.
		 */
		cmm_smp_mb();
		backend_pages = lib_ring_buffer_offset_backend_pages(&buf->backend,
					idx * chan->backend.subbuf_size, handle);
		if (!backend_pages)
			return;
		deliver_count_events(config, buf, idx, handle);
		config->cb.buffer_end(buf, backend_pages->timestamp_end, idx,
