 ...
 *
 * LTTng UST bytecode specializer for side events.
 *
 * Adapted from lttng-bytecode-specialize.c: payload field lookups
 * resolve against the side event description to a side argument
 * index (get_index data offset = argument index, field = NULL),
 * and object field loads are never specialized into the memory-based
 * width-specific load instructions: they stay on the generic dynamic
 * LOAD_FIELD, which the side interpreter resolves through the
 * self-describing SIDE payload ABI (payload objects) or the context
 * field types (context objects, get_index data field != NULL).
 */
		...
		size_t align, size_t len)
{
	...
	if (new_alloc_len > old_alloc_len) {
		...
		new_alloc_len =
			max_t(size_t, 1U << lttng_ust_get_count_order_ulong(new_alloc_len), old_alloc_len << 1);
		...
		struct load_op *insn)
{
	...
	/* Side: object loads stay on the generic dynamic LOAD_FIELD. */
	(void) insn;
...
static int specialize_side_load_object(const struct side_type *side_type,
		...
		int idx_len)
{
	...
	case LOAD_OBJECT:
		...
		case OBJECT_TYPE_SEQUENCE:
			/*
			 * Side arrays and VLAs are nested argument
			 * vectors: the index is an element index rather
			 * than a byte offset, and each element carries
			 * its own type, which is therefore only known
			 * when the event is emitted. Bounds are checked
			 * by the interpreter against the argument vector
			 * length, which also covers the fixed-length
			 * array case.
			 */
			if (stack_top->load.field) {
				ERR("Index lookup within a context array is not supported.");
				ret = -EINVAL;
				goto end;
			}
			gid.offset = index;	/* side element index */
			/*
			 * The byte order of the elements was taken from
			 * the element type when the array or the VLA was
			 * loaded, and applies to the element indexed
			 * here.
			 */
			gid.elem.rev_bo = stack_top->load.rev_bo;
			if (stack_top->load.side_type
					&& side_type_is_gather_container(stack_top->load.side_type)) {
				const struct side_type *elem_type =
					side_gather_container_elem_type(stack_top->load.side_type);

				/*
				 * The elements of a gathered container
				 * are typed by the description rather
				 * than by an argument of their own, so
				 * the interpreter reaches them through
				 * the type of the container.
				 */
				gid.side_type = stack_top->load.side_type;
				ret = specialize_side_load_object(elem_type, &stack_top->load);
				if (ret)
					goto end;
			...
			/*
			 * The elements of a stack-copy container are
			 * arguments which carry their own type, so the
			 * value loaded from one is typed by the
			 * argument. Their type is known all the same,
			 * and a symbol resolved against an element
			 * needs it.
			 */
			gid.elem.type = OBJECT_TYPE_DYNAMIC;
			stack_top->load.object_type = OBJECT_TYPE_DYNAMIC;
			stack_top->load.side_type =
				side_container_elem_type(stack_top->load.side_type);
			gid.side_type = stack_top->load.side_type;
			break;
		case OBJECT_TYPE_STRUCT:
...
/*
 * Map a side payload field type to the object/register type used for
 * type tracking and comparator specialization. Only the field types
 * mapped by the side tracer integration are supported.
 */
/*
 * The trace records a value in the byte order it is emitted with, but
 * the interpreter compares values, so it converts to the byte order of
 * the host. Integers and floats do not answer to the same host order.
 */
static bool side_integer_rev_bo(const struct side_type_integer *type_integer)
{
	return side_enum_get(type_integer->byte_order) != SIDE_TYPE_BYTE_ORDER_HOST;
}

static bool side_float_rev_bo(const struct side_type_float *type_float)
{
	return side_enum_get(type_float->byte_order) != SIDE_TYPE_FLOAT_WORD_ORDER_HOST;
}

/*
 * The elements of an array or of a VLA all have the description's
 * element type, so their byte order is known even though their value
 * is only typed when the event is emitted.
 */
static bool side_elem_rev_bo(const struct side_type *elem_type)
{
	...
	case SIDE_TYPE_POINTER:
		return side_integer_rev_bo(&elem_type->u.side_integer);
	case SIDE_TYPE_FLOAT_BINARY32:	/* Fall-through. */
	case SIDE_TYPE_FLOAT_BINARY64:
		return side_float_rev_bo(&elem_type->u.side_float);
	case SIDE_TYPE_ENUM:
	{
		...
		struct vstack_load *load)
{
	load->type = LOAD_OBJECT;
	/*
	 * Payload objects are side arguments: field is left NULL
	 * (context objects carry their context event field), and
	 * loads stay on the generic dynamic LOAD_FIELD.
	 */
	load->field = NULL;
	load->side_type = side_type;
	/*
	 * The byte order to convert from when a value is loaded from
	 * this object: its own for a scalar, that of the elements for
	 * an array or a VLA, whose elements are the values loaded from
	 * it.
	 */
	load->rev_bo = false;
	...
	case SIDE_TYPE_S64:
		load->object_type = OBJECT_TYPE_S64;
		load->rev_bo = side_integer_rev_bo(&side_type->u.side_integer);
		break;
	case SIDE_TYPE_U8:		/* Fall-through. */
	case SIDE_TYPE_U16:		/* Fall-through. */
	case SIDE_TYPE_U32:		/* Fall-through. */
	...
	case SIDE_TYPE_BYTE:
		/* A boolean argument is a normalized byte, as is a byte. */
		load->object_type = OBJECT_TYPE_U64;
		break;
	case SIDE_TYPE_FLOAT_BINARY32:	/* Fall-through. */
	case SIDE_TYPE_FLOAT_BINARY64:
		load->object_type = OBJECT_TYPE_DOUBLE;
		load->rev_bo = side_float_rev_bo(&side_type->u.side_float);
		break;
	case SIDE_TYPE_STRING_UTF8:
		load->object_type = OBJECT_TYPE_STRING;
		break;
	/*
	 * A structure is not loaded as a value: it is descended into,
	 * by a symbol which names one of its fields.
	 */
	case SIDE_TYPE_STRUCT:		/* Fall-through. */
	case SIDE_TYPE_GATHER_STRUCT:
		load->object_type = OBJECT_TYPE_STRUCT;
		break;
	 ...
	 * arguments of their own: they are read from the memory the
	 * container resolves to, one element size apart. The type of
	 * the container is what the interpreter needs to reach them.
	 */
	...
	case SIDE_TYPE_ENUM:
	{
		...
		load->rev_bo = side_integer_rev_bo(&container->u.side_integer);
		break;
	}
	default:
...
/*
 * A symbol resolved against an object rather than against the payload:
 * the field of a structure. It becomes an index of the member within
 * that structure, which the interpreter descends into, so the type of
 * the structure travels with it.
 */
static int specialize_side_object_lookup(struct bytecode_runtime *runtime,
		struct load_op *insn, struct vstack_load *load)
{
	const struct side_type *side_type = load->side_type;
	const struct side_type *member_type = NULL;
	struct bytecode_get_index_data gid;
	ssize_t data_offset;
	const char *name;
	uint16_t offset;
	...
	ret = side_struct_member_lookup_by_name(side_type, name, &idx, &member_type);
	if (ret)
		return -EINVAL;

	memset(&gid, 0, sizeof(gid));
	gid.offset = idx;		/* member index */
	gid.side_type = side_type;	/* the structure descended into */
	gid.field = NULL;
	ret = specialize_side_load_object(member_type, load);
	if (ret)
		return ret;
	gid.elem.type = load->object_type;
	gid.elem.rev_bo = load->rev_bo;
...
static int specialize_payload_lookup(const struct side_event_description *side_desc,
		struct bytecode_runtime *runtime,
		struct load_op *insn,
		struct vstack_load *load)
{
	...
	offset = ((struct get_symbol *) insn->data)->offset;
	name = runtime->p.bc->bc.data + runtime->p.bc->bc.reloc_offset + offset;
	idx = lttng_bytecode_side_field_lookup(side_desc, name, &side_type);
	if (idx < 0) {
		ret = -EINVAL;
		goto end;
	}

	ret = specialize_side_load_object(side_type, load);
	if (ret)
		goto end;
	...
	insn->op = BYTECODE_OP_GET_INDEX_U16;
	memset(&gid, 0, sizeof(gid));
	gid.offset = idx;	/* side argument index */
	gid.elem.type = load->object_type;
	gid.elem.rev_bo = load->rev_bo;
	...
	for (pc = next_pc = start_pc; pc - start_pc < bytecode->len;
		...
		case BYTECODE_OP_GET_SYMBOL:
		{
			...
			case LOAD_OBJECT:
				...
				ret = specialize_side_object_lookup(bytecode,
					insn, &vstack_ax(stack)->load);
				if (ret)
					goto end;
				break;
			case LOAD_ROOT_CONTEXT:
			...
			case LOAD_ROOT_PAYLOAD:
				...
				ret = specialize_payload_lookup(side_desc,
					bytecode, insn,
					&vstack_ax(stack)->load);
				if (ret)
					goto end;
