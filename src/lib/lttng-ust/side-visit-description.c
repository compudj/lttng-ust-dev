 ...
 * distance where the type was defined alongside it, and an address
 * where it was defined in another translation unit, which the assembler
 * cannot fold a distance to. Both resolve here without help, the
 * description being read in the process which wrote it. See
 * side_ptr_sel_t.
 */
#define visit_side_sel_pointer(visitor, ptr)	side_ptr_sel_get(ptr)
