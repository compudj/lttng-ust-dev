...
static
void trace_dlopen(const struct bin_info_data *bin_data, void *ip)
{
...
static
void trace_dlclose(const struct bin_info_data *bin_data, void *ip)
{
	tracepoint(lttng_ust_dl, dlclose, ip, bin_data->base_addr_ptr);
}
...
void iter_end(struct dl_iterate_data *data, void *ip)
{
	 ...
	 * For each marked, not traced, trace dlopen event. traced = true.
	 * For each unmarked, traced, trace dlclose event. remove node.
	...
	for (i = 0; i < UST_DL_STATE_TABLE_SIZE; i++) {
		...
		cds_hlist_for_each_entry_2(e, head, node) {
			if (e->marked) {
				if (!e->traced) {
					trace_dlopen(&e->bin_data, ip);
					e->traced = true;
				}
			} else {
				if (e->traced)
					trace_dlclose(&e->bin_data, ip);
				remove_dl_node(e);
				free_dl_node(e);
			}
			e->marked = false;
