 ...
 * lttng-ust-rcu.c
 *
 * LTTng-UST RCU implementation
 *
 * Inspired from the Userspace RCU library, "bulletproof" RCU flavor.
 *
...
#define lttng_ust_rcu_read_lock 	_lttng_ust_rcu_read_lock
#define lttng_ust_rcu_read_unlock	_lttng_ust_rcu_read_unlock
#define lttng_ust_rcu_read_ongoing	_lttng_ust_rcu_read_ongoing
#include <lttng/ust-rcu.h>
#undef lttng_ust_rcu_read_ongoing
#undef lttng_ust_rcu_read_unlock
#undef lttng_ust_rcu_read_lock

#define lttng_ust_die(cause)								\
do {										\
...
#define ARENA_INIT_ALLOC		\
	...
	+ INIT_NR_THREADS * sizeof(struct lttng_ust_rcu_reader)
...
static
void __attribute__((constructor)) _lttng_ust_rcu_init(void);
static
void __attribute__((destructor)) lttng_ust_rcu_exit(void);

#ifndef CONFIG_LTTNG_UST_FORCE_SYS_MEMBARRIER
int lttng_ust_rcu_has_sys_membarrier;
#endif
...
static pthread_key_t lttng_ust_rcu_key;

struct lttng_ust_rcu_gp lttng_ust_rcu_gp = { .ctr = LTTNG_UST_RCU_GP_COUNT };

/*
 * Pointer to registry elements. Written to only by each individual reader. Read
 * by both the reader and the writers.
 */
DEFINE_URCU_TLS(struct lttng_ust_rcu_reader *, lttng_ust_rcu_reader);
			...
			struct cds_list_head *qsreaders)
{
	...
	struct lttng_ust_rcu_reader *index, *tmp;

	/*
	 * Wait for each thread URCU_TLS(lttng_ust_rcu_reader).ctr to either
	 * indicate quiescence (not nested), or observe the current
	 * lttng_ust_rcu_gp.ctr value.
	 */
	for (;;) {
		...
		cds_list_for_each_entry_safe(index, tmp, input_readers, node) {
			switch (lttng_ust_rcu_reader_state(&index->ctr)) {
			case LTTNG_UST_RCU_READER_ACTIVE_CURRENT:
				if (cur_snap_readers) {
				...
				/* Fall-through */
			case LTTNG_UST_RCU_READER_INACTIVE:
				cds_list_move(&index->node, qsreaders);
				break;
			case LTTNG_UST_RCU_READER_ACTIVE_OLD:
				/*
...
void lttng_ust_synchronize_rcu(void)
{
	...
	CMM_STORE_SHARED(lttng_ust_rcu_gp.ctr, lttng_ust_rcu_gp.ctr ^ LTTNG_UST_RCU_GP_CTR_PHASE);
...
void lttng_ust_rcu_read_lock(void)
{
	_lttng_ust_rcu_read_lock();
}

void lttng_ust_rcu_read_unlock(void)
{
	_lttng_ust_rcu_read_unlock();
}

int lttng_ust_rcu_read_ongoing(void)
{
	return _lttng_ust_rcu_read_ongoing();
}
...
void expand_arena(struct registry_arena *arena)
{
	...
	if (cds_list_empty(&arena->chunk_list)) {
		assert(ARENA_INIT_ALLOC >=
			sizeof(struct registry_chunk)
			+ sizeof(struct lttng_ust_rcu_reader));
...
static
struct lttng_ust_rcu_reader *arena_alloc(struct registry_arena *arena)
{
	struct registry_chunk *chunk;
	struct lttng_ust_rcu_reader *rcu_reader_reg;
	int expand_done = 0;	/* Only allow to expand once per alloc */
	size_t len = sizeof(struct lttng_ust_rcu_reader);
...
retry:
	cds_list_for_each_entry(chunk, &arena->chunk_list, node) {
		if (chunk->data_len - chunk->used < len)
			continue;
		/* Find spot */
		for (rcu_reader_reg = (struct lttng_ust_rcu_reader *) &chunk->data[0];
				rcu_reader_reg < (struct lttng_ust_rcu_reader *) &chunk->data[chunk->data_len];
				rcu_reader_reg++) {
...
void add_thread(void)
{
	...
	if (!rcu_reader_reg)
		abort();
	ret = pthread_setspecific(lttng_ust_rcu_key, rcu_reader_reg);
	if (ret)
		abort();
	 ...
	 */
	URCU_TLS(lttng_ust_rcu_reader) = rcu_reader_reg;
}
		...
		struct lttng_ust_rcu_reader *rcu_reader_reg)
{
	...
	chunk->used -= sizeof(struct lttng_ust_rcu_reader);
}

static
struct registry_chunk *find_chunk(struct lttng_ust_rcu_reader *rcu_reader_reg)
{
	struct registry_chunk *chunk;

	cds_list_for_each_entry(chunk, &registry_arena.chunk_list, node) {
		if (rcu_reader_reg < (struct lttng_ust_rcu_reader *) &chunk->data[0])
			continue;
		if (rcu_reader_reg >= (struct lttng_ust_rcu_reader *) &chunk->data[chunk->data_len])
			continue;
		return chunk;
	}
	return NULL;
...
/* Called with signals off and mutex locked */
static
void remove_thread(struct lttng_ust_rcu_reader *rcu_reader_reg)
{
	cleanup_thread(find_chunk(rcu_reader_reg), rcu_reader_reg);
	URCU_TLS(lttng_ust_rcu_reader) = NULL;
}

/* Disable signals, take mutex, add to registry */
void lttng_ust_rcu_register(void)
{
	sigset_t newmask, oldmask;
	int ret;
	 ...
	 */
	if (URCU_TLS(lttng_ust_rcu_reader))
		goto end;
...
static
void lttng_ust_rcu_unregister(struct lttng_ust_rcu_reader *rcu_reader_reg)
{
	sigset_t newmask, oldmask;
	int ret;
 ...
 */
static
void lttng_ust_rcu_thread_exit_notifier(void *rcu_key)
{
	lttng_ust_rcu_unregister(rcu_key);
}

#ifdef CONFIG_LTTNG_UST_FORCE_SYS_MEMBARRIER
static
void lttng_ust_rcu_sys_membarrier_status(bool available)
{
	if (!available)
		lttng_ust_die(ENOSYS);
}
#else
static
void lttng_ust_rcu_sys_membarrier_status(bool available)
{
	if (!available)
		return;
	lttng_ust_rcu_has_sys_membarrier = 1;
}
#endif

static
void lttng_ust_rcu_sys_membarrier_init(void)
{
	...
	lttng_ust_rcu_sys_membarrier_status(available);
}

static
void _lttng_ust_rcu_init(void)
{
	mutex_lock(&init_lock);
	if (!lttng_ust_rcu_refcount++) {
		int ret;

		ret = pthread_key_create(&lttng_ust_rcu_key,
				lttng_ust_rcu_thread_exit_notifier);
		if (ret)
			abort();
		lttng_ust_rcu_sys_membarrier_init();
		initialized = 1;
	}
...
void lttng_ust_rcu_exit(void)
{
	...
	if (!--lttng_ust_rcu_refcount) {
		...
		ret = pthread_key_delete(lttng_ust_rcu_key);
		if (ret)
			abort();
	}
	mutex_unlock(&init_lock);
 ...
 */
void lttng_ust_rcu_before_fork(void)
{
	sigset_t newmask, oldmask;
	int ret;
...
void lttng_ust_rcu_after_fork_parent(void)
{
	sigset_t oldmask;
	int ret;
 ...
 */
static
void lttng_ust_rcu_prune_registry(void)
{
	struct registry_chunk *chunk;
	struct lttng_ust_rcu_reader *rcu_reader_reg;
...
void lttng_ust_rcu_after_fork_child(void)
{
	sigset_t oldmask;
	int ret;

	lttng_ust_rcu_prune_registry();
	oldmask = saved_fork_signal_mask;
	mutex_unlock(&rcu_registry_lock);
	mutex_unlock(&rcu_gp_lock);
	ret = pthread_sigmask(SIG_SETMASK, &oldmask, NULL);
...
const struct rcu_flavor_struct lttng_ust_rcu_flavor = {
	.read_lock		= lttng_ust_rcu_read_lock,
	.read_unlock		= lttng_ust_rcu_read_unlock,
	.read_ongoing		= lttng_ust_rcu_read_ongoing,
	.read_quiescent_state	= NULL,
	.update_call_rcu	= NULL,
	.update_synchronize_rcu	= lttng_ust_synchronize_rcu,
	.update_defer_rcu	= NULL,
	.thread_offline		= NULL,
	.thread_online		= NULL,
	.register_thread	= NULL,
	.unregister_thread	= NULL,
	.barrier		= NULL,
	.register_rculfhash_atfork = NULL,
	.unregister_rculfhash_atfork = NULL,
};
