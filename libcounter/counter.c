	...
	const int *counter_cpu_fds)
{
	...
	if ((config->alloc & COUNTER_ALLOC_PER_CPU)) {
		if (nr_counter_cpu_fds != lttng_counter_num_possible_cpus())
			return -1;
		if (!counter_cpu_fds)
			return -1;
	} else {
					 ...
					 bool is_daemon)
{
	...
	if (config->alloc & COUNTER_ALLOC_PER_CPU) {
		counter->percpu_counters = zmalloc(sizeof(struct lib_counter_layout) * nr_counter_cpu_fds);
